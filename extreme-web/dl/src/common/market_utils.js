...
import {ChainStore, ChainTypes} from "extremejs-lib";
...
class MarketUtils {
    ...
    static parseOrder(order, base, quote, invert = false) {
        ...
        if (order.debt) {
            if (invert) {
                // Price in USD/XTR, amount should be in XTR, value should be in USD, debt is in USD
                // buy is in USD, sell is in XTR
                // quote is USD, base is XTR
                value = order.debt / quotePrecision;
                amount = this.limitByPrecision(value / price.full, base);
            } else {
                // Price in XTR/USD, amount should be in USD, value should be in XTR, debt is in USD
                // buy is in USD, sell is in XTR
                // quote is USD, base is XTR
